                                ...
                                <figure class="md:flex rounded-xl md:p-0">
                                    ...
                                    <div class="w-full pt-6 p-5 md:p-3 text-center md:text-left space-y-4">
                                        ...
                                        <div v-if="donation.status == 'pending'">
                                            <button @click="payment(donation.snap_token)" class="w-full bg-gray-600 text-white rounded shadow-sm text-xs py-1 px-2 font-bold focus:outline-none">Lanjutkan Pembayaran</button>
                                        </div>
                                    </div>
                                    <div class="ml-auto text-sm text-gray-500 underline">
                                        <div v-if="donation.status == 'success'">
                                            <button class="bg-gray-500 border-2 border-green-500 rounded shadow-sm text-xs py-1 px-2 text-white focus:outline-none">Berhasil</button>
                                        </div>
                                        <div v-else-if="donation.status == 'pending'">
                                            <button class="bg-gray-500 text-white border-2 rounded shadow-sm text-xs py-1 px-2 font-bold focus:outline-none">Pending</button>
                                        </div>
                                        <div v-else-if="donation.status == 'expired'">
                                            <button class="bg-red-500 border-2 rounded shadow-sm text-xs py-1 px-2 text-white focus:outline-none">Dibatalkan</button>
                                        </div>
